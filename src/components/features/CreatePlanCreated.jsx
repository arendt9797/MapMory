      ...
      <form onSubmit={handleSavePlan}>
        <div className="grid border-2 border-primary rounded-md">
          <div className="border-b-2 border-primary p-5 flex items-center justify-center">
            <h1 className="text-lg text-secondary mr-2 font-bold min-w-fit">계획 이름 : </h1>
            <input
              type="text"
          ...
          <ul className="p-6 space-y-6 overflow-y-auto h-80 scrollbar scrollbar-thumb-primary scrollbar-track-transparent">
            {detailPlans.map((detailPlan, idx) => (
              <li key={idx} className="flex items-center gap-4">
                ...
                <div>
                  <Title theme='secondary' size={''} >{detailPlan.planMemo}</Title>
                  <div className="text-sm text-secondaryHover">
                    <Text theme={'secondary'} size={'md'} className="inline mr-4">날짜 : {detailPlan.planDate}</Text>
                    <Text theme={'secondary'} size={'md'} className='inline'>시간 : {detailPlan.planTime}</Text>
                  </div>
                </div>
                <Button type="button" className="bg-red-400 px-0 py-0 w-6 h-6" onClick={() => onHandleDeleteMarker(idx)} >
                  &times;
                </Button>
              </li>
            ))}
